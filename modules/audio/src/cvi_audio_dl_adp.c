...
CVI_S32 CVI_Audio_Dlopen(CVI_VOID **pLibhandle, CVI_CHAR *pChLibName)
{
    ...
    *pLibhandle = dlopen(pChLibName, RTLD_LAZY | RTLD_LOCAL);
    ...
    {
        APP_PROF_LOG_PRINT(LEVEL_ERROR, "dlopen %s failed! [error]%s\n", pChLibName, dlerror());
        return CVI_FAILURE;
    }
            ...
            CVI_CHAR *pChFuncName)
{
    ...
    {
        APP_PROF_LOG_PRINT(LEVEL_ERROR, "LibHandle is empty!");
        return CVI_FAILURE;
    }
    ...
    *pFunchandle = dlsym(Libhandle, pChFuncName);
    ...
    {
        APP_PROF_LOG_PRINT(LEVEL_ERROR, "dlsym %s fail,error msg is %s!\n", pChFuncName, dlerror());
        return CVI_FAILURE;
    }
